...
async def ingest_folder_documents(folder: str, config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if use_gcp:
            # Lógica para GCP (mantener código existente)
            from google.cloud import storage
            return await _ingest_from_gcp(folder, config, storage, DocumentLoader, RAGPipeline)
        else:
            # Lógica para almacenamiento local
            return await _ingest_from_local(folder, config, DocumentLoader, RAGPipeline)
...
async def _ingest_from_local(folder: str, config: Dict[str, Any], DocumentLoader, RAGPipeline) -> bool:
    """
    ...
    try:
        ...
        rag_pipeline = RAGPipeline(config, is_production=True)
        ...
        for file_path in files:
            try:
                ...
                # Usar DocumentLoader para procesar
                documents = DocumentLoader.load_document_content(file_content, os.path.basename(file_path))
                
                if documents:
                    # Agregar documentos al pipeline
                    await rag_pipeline.add_documents(documents, collection_name)
                    processed_count += 1
                    logger.info(f"✅ Archivo procesado: {os.path.basename(file_path)}")
                else:
                    logger.warning(f"⚠️ No se pudo procesar: {os.path.basename(file_path)}")
...
async def _ingest_from_gcp(folder: str, config: Dict[str, Any], storage, DocumentLoader, RAGPipeline) -> bool:
    """
